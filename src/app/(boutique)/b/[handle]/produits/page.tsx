...
export default async function CatalogPage({
  ...
  const [collections, categories, totalCount, products] = await Promise.all([
  ...
  function pageHref(targetPage: number) {
    ...
    return query ? `${base}/produits?${query}` : `${base}/produits`;
  }
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        ...
        {categories.map((category) => (
          <FilterChip
            ...
            href={`${base}/produits?category=${category.slug}`}
          />
        ))}
      </div>
